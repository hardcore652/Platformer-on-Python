...
script_dir = script_dir.replace('level_test.py', '')
...
with open(script_dir+"editor_lvl.txt", "r") as f:
	current_lvl = f.readline()
remove(script_dir+"editor_lvl.txt")
...
while 1:
	...
	for i in event.get(): # управление игроком
		...
		if i.type == KEYDOWN and i.key == K_RETURN:
			pygame.quit()
			call(['python', script_dir + 'level_editor.py'])
	...
	help_text_img = font.render('PRESS ENTER TO RETURN IN EDITOR', 1, (0, 255, 0))
	screen.blit(help_text_img, (285, 5))
